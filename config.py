...
def setup_config(translator=None):
    ...
    try:
        ...
        # normalize path
        config_dir = os.path.normpath(os.path.join(docs_path, ".cursor-pro"))
        ...
        try:
            ...
            import tempfile
            temp_dir = os.path.normpath(os.path.join(tempfile.gettempdir(), ".cursor-pro"))
        ...
        default_config = {
            'Browser': {
                ...
                'operagx_driver_path': get_default_driver_path('chrome')  # Opera GX su dung Chrome driver
            },
            'Turnstile': {
            ...
            'OAuth': {
                'show_selection_alert': False,  # Mac dinh khong hien thi thong bao chon
                'timeout': 120,
                'max_attempts': 3
            },
            'Token': {
...
def print_config(config, translator=None):
    ...
    for section in config.sections():
        ...
        for key, value in config.items(section):
            # Xu ly dac biet cho gia tri boolean, de hien thi mau sac
    ...
    print(f"\n{Fore.CYAN}{'─' * 70}{Style.RESET_ALL}")
    config_dir = os.path.join(get_user_documents_path(), ".cursor-pro", "config.ini")
