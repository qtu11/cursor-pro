...
def get_cursor_paths(translator=None) -> Tuple[str, str]:
    ...
    if not config.has_section(section) or not config.has_option(section, 'cursor_path'):
        raise OSError(translator.get('reset.path_not_configured') if translator else "Chua cau hinh duong dan Cursor")
    ...
    if not os.path.exists(base_path):
        raise OSError(translator.get('reset.path_not_found', path=base_path) if translator else f"Khong tim thay duong dan Cursor: {base_path}")
    ...
    if not os.path.exists(pkg_path):
        raise OSError(translator.get('reset.package_not_found', path=pkg_path) if translator else f"Khong tim thay package.json: {pkg_path}")
    if not os.path.exists(main_path):
        raise OSError(translator.get('reset.main_not_found', path=main_path) if translator else f"Khong tim thay main.js: {main_path}")
    ...
    if system == "Linux":
        ...
        for base in paths_map["Linux"]["bases"]:
            ...
            if os.path.exists(main_path):
                return main_path
        raise OSError(translator.get('reset.linux_path_not_found') if translator else "Khong tim thay duong dan cai dat Cursor tren he thong Linux")
